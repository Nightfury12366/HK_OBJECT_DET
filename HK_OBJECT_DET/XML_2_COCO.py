...
ap.add_argument('-c', '--compound_coef', type=int, default=5, help='coefficients of efficientdet')
    ...
    for i in root:  # 遍历一级节点
        ...
        if i.tag == 'object':  # 目标
            for j in i:
                ...
                if cls_name in voc_clses:  # 如果这个类别在需要检测的目标列表中，才进行添加
...
if __name__ == '__main__':
    ...
    for i_index, xml_file in enumerate(xmls):
        ...
        if len(sig_xml_bbox) != 0:  # 这张图片包含需要的物体才把它加到数据集之中
            # print('sig_img_box', sig_xml_bbox)
    ...
    print('生成的'+project_name+'数据图片数量： ', len(images))
    ...
    json.dump(ann_js, open(json_name, 'w', encoding='utf-8'), ensure_ascii=False, indent=4)  # indent=4 更加美观显示
